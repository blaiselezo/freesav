export class SwadeCombat extends Combat {
  actionCardDeck: RollTable;
  actionCardPack: Compendium;

  constructor(data: BaseEntityData, options: any) {
    super(data, options);

    this.actionCardDeck = game.tables.getName(CONFIG.SWADE.init.cardTable);
    this.actionCardPack = game.packs.get(
  ...
  async rollInitiative(
    ...
    if (
      ids.length > this.actionCardDeck.results.filter((r) => !r.drawn).length
    ) {
   ...
   */
  async drawCard(count?: number): Promise<JournalEntry[]> {
    if (
      this.actionCardPack === null ||
      (await this.actionCardPack.getIndex()).length === 0
    ) {
      console.log(
    ...
    const packIndex = await this.actionCardPack.getIndex();
    const cards: JournalEntry[] = [];
    if (!count) count = 1;

    for (let i = 0; i < count; i++) {
      let drawResult = await this.actionCardDeck.draw({ displayChat: false });
      const lookUpCard = packIndex.find(
        (c) => c.name === drawResult.results[0].text,
      );
      cards.push(
        (await this.actionCardPack.getEntity(lookUpCard._id)) as JournalEntry,
      );
    }
    return cards;
  }
  ...
  async findCard(cardValue: number, cardSuit: number): Promise<JournalEntry> {
    const content = (await this.actionCardPack.getContent()) as JournalEntry[];
    return content.find(
      (c) =>
